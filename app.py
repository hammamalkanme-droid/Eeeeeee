...
def get_main_keyboard(user_id):
    ...
    markup.add(types.KeyboardButton("إعدادات البوست"), types.KeyboardButton("مشاركة وتفعيل"))
...
def send_welcome(message):
    ...
    welcome_text = (
        f"مرحباً بك عزيزي {message.from_user.first_name} في نظام إدارة الحضور المطور\n\n"
        "هنا يمكنك إنشاء بوستات الحضور بكل احترافية، متابعة التفاعلات، وجلب الزوار عبر رابطك الخاص.\n\n"
        "رابط دعوتك الشخصي:\n"
...
def handle_menu_buttons(message):
    ...
    if message.text == "إعدادات البوست":
        ...
        bot.reply_to(message, "أرسل الآن عنوان أو كليشة البوست التي ستظهر للمستخدمين عند نشر زر الحضور:")
        ...
        if not top_users:
            ...
            for i, (uid, count) in enumerate(top_users):
                ...
                leaderboard_text += f"{medal}. الأيدي (ID: {uid}) — {count} زائر\n"
        bot.reply_to(message, leaderboard_text)
    elif message.text == "الدعم والمساعدة":
        user_states[user_id] = "waiting_support_msg"
        bot.reply_to(message, "أرسل رسالتك أو استفسارك الآن، وسيتم تحويله مباشرة إلى الإدارة للرد عليك:")
...
def admin_start_reply(call):
    ...
    bot.send_message(ADMIN_ID, f"اكتب الرد الذي تريد إرساله للمستخدم (ID: {target_user_id}):")
...
def handle_channel_attendance(call):
    ...
    if cursor.fetchone():
        bot.answer_callback_query(call.id, "لقد قمت بتسجيل حضورك مسبقاً!", show_alert=True)
        conn.close()
        return
